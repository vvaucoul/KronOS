...
int vfs_init(void) {
    vfs_mounts = hashtable_create(VFS_MAX_FS);
    if (vfs_mounts == NULL) {
...
int vfs_mount(Vfs *vfs) {
    if (vfs == NULL || vfs->fsops == NULL || vfs->fsops->mount == NULL) {
        ...
        if ((hashtable_contains_key(vfs_mounts, vfs->fs_info->name)) == 0) {
            return (1);
        }
    }
    return (0);
...
int vfs_unmount(Vfs *vfs) {
    if (vfs == NULL || vfs->fsops == NULL || vfs->fsops->unmount == NULL) {
        ...
        if ((hashtable_contains_key(vfs_mounts, vfs->fs_info->name)) == 1) {
            return (1);
        }
    }
    return (0);
