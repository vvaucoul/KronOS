...
/*   Updated: 2024/07/31 14:35:25 by vvaucoul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int __kheap_expand_heap(uint32_t new_size, heap_t *heap) {
	/* Check if new size is less than current size */
	if (new_size <= heap->addr.end_address - heap->addr.start_address) {
		return 1;
	}

	new_size = (new_size & 0xFFFFF000) ? (new_size & 0xFFFFF000) + PAGE_SIZE : new_size;
	if (heap->addr.start_address + new_size > heap->addr.max_address) {
		return 1;
	}
	...
	/* Check if next address is greater than max address */
	uint32_t next_address = old_end_address + sizeof(heap_header_t) + sizeof(heap_footer_t);
	if (next_address >= heap->addr.end_address) {
		return 1;
	}

	for (uint32_t addr = old_end_address; addr < heap->addr.end_address; addr += PAGE_SIZE) {
		page_t *page = get_page(addr, kernel_directory);
		if (!page) {
			page = create_page(addr, kernel_directory);
			if (!page) {
		...
		if (!page->present) {
			...
			if (!page->frame) {
				__PANIC("Failed to allocate frame for page");
			}
		}
	}

	printk("Heap expanded from 0x%x to 0x%x\n", old_end_address, heap->addr.end_address);
	return 0;
}
...
data_t kheap_alloc(uint32_t size, bool align, heap_t *heap) {
	...
	if (iterator == -1) {
		...
		uint32_t old_end_address = heap->addr.end_address;
		if ((__kheap_expand_heap(old_length + new_size, heap)) == 1) {
			return NULL;
		}
